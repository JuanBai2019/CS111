...
void cmd_line_handler(char *buf_cmd_line)
{
    ...
    {
        if(strlen(buf_cmd_line) > 4) //there are required "line of text" after LOG option (command is valid)
        {
            if(log_flag == 1)
            {
                dprintf(log_fd, "%s\n", buf_cmd_line);
        ...
        {
            fprintf(stderr, "Error, argument LOG is invalid\n");
            exit(1);
        }
    }
...
int main(int argc, char *argv[])
{
    ...
    while(1)
    {
        ...
        if(val_returned > 0)
        {
            int ret = read(sockfd, buf_cmd_line, 20);
            if(ret > 0)
            {
